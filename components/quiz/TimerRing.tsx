...
export function TimerRing({ secondsLeft, totalSeconds, size = "normal" }: { secondsLeft: number; totalSeconds: number; size?: "normal" | "stage" }) {
  ...
  return (
    <div
      className={cn(
        "stage-timer grid place-items-center rounded-full font-black text-white transition-all duration-300",
    ...
    >
      <div className={cn("stage-timer-inner grid place-items-center rounded-full bg-show-navy transition-colors duration-300", stage ? "h-24 w-24" : "h-16 w-16", urgent && "border border-show-red/70 text-show-red")}>
        {secondsLeft}
      </div>
    </div>
  );
