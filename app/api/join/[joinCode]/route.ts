...
import { normalizeAvatarId } from "@/lib/participant-avatars";
...
export async function POST(request: Request, { params }: { params: Promise<{ joinCode: string }> }) {
  ...
  if (!isAllowedParticipantEmoji(emoji)) return NextResponse.json({ error: "Bitte ein Emoji aus der Auswahl wählen." }, { status: 400 });
  const result = await joinSession(joinCode, displayName, emoji, avatarId);
